...
const getOptionalEnv = (key: string, fallback = "") => process.env[key] ?? fallback;
...
export const PORT = getOptionalNumberEnv("PORT", 8000);
...
export const envs = {
  ...
  MAILHOG_SMTP_HOST: getOptionalEnv("MAILHOG_SMTP_HOST", "127.0.0.1"),
  MAILHOG_SMTP_PORT: getOptionalNumberEnv("MAILHOG_SMTP_PORT", 1025),
  GMAIL_USER: getOptionalEnv("GMAIL_USER"),
  GMAIL_PASS: getOptionalEnv("GMAIL_PASS"),
  EMAIL_FROM: getOptionalEnv("EMAIL_FROM", "Edge <no-reply@edge.dev>"),
  GOOGLE_CLIENT_ID,
  GOOGLE_CLIENT_SECRET,
  GOOGLE_CALLBACK_URL,
  GITHUB_CLIENT_ID,
